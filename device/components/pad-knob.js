...
class PadKnob extends HTMLElement {
    ...
    render() {
        ...
        this.shadowRoot.innerHTML = `
            <style>
                ...
                .knob-container {
                    position: relative;
                    width: min(var(--knob-size, 120px), 100%);
                    aspect-ratio: 1 / 1;
                    max-height: 100%;
                    flex-shrink: 0;
                    margin: 0 auto;
                ...
                :host(.no-transition) .knob-indicator {
    ...
    setupEventListeners() {
        ...
        const handleMove = (e) => {
            ...
            let clientY;
            if (e.type.includes('touch')) {
                // Find the touch matching this knob's touchId
        ...
        const handleEnd = (e) => {
            if (e.type.includes('touch')) {
                // Only end if this touch matches our touchId
                const endedTouch = Array.from(e.changedTouches).find(t => t.identifier === this.touchId);
