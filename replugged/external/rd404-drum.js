// RD404Drum - Simple 4-Voice WASM Drum Synthesizer
...
class RD404Drum {
    ...
    async initialize() {
        console.log('[RD404Drum] 🥁 Initializing WASM Drum Synth...');
        try {
            ...
            console.log('[RD404Drum] Audio graph connected: worklet → masterGain → speakerGain → destination');
            ...
            this.workletNode.port.onmessage = (event) => {
                ...
                if (type === 'needWasm') {
                    ...
                    console.log('[RD404Drum] ✅ WASM Drum Synth ready');
                    this.wasmReady = true;
                } else if (type === 'error') {
                    console.error('[RD404Drum] WASM error:', data);
                }
            };

            this.isActive = true;

            console.log('[RD404Drum] Initialized - waiting for WASM...');
            return true;
        } catch (error) {
            console.error('[RD404Drum] Failed to initialize:', error);
            return false;
        }
    }
    ...
    async loadWasm() {
        try {
            console.log('[RD404Drum] Loading WASM...');

            // Fetch both JS glue code and WASM binary (cache-busting with timestamp)
            const timestamp = Date.now();
            const [jsResponse, wasmResponse] = await Promise.all([
                fetch(`${window.location.pathname.includes('/rfxsynths/') ? '' : '../rfxsynths/'}rd404-drum.js?t=${timestamp}`),
                fetch(`${window.location.pathname.includes('/rfxsynths/') ? '' : '../rfxsynths/'}rd404-drum.wasm?t=${timestamp}`)
            ]);
            ...
            console.log('[RD404Drum] WASM sent to worklet');
        } catch (error) {
            console.error('[RD404Drum] Failed to load WASM:', error);
        }
    }
    ...
    async setAudible(enabled) {
        if (!this.speakerGain) {
            console.error('[RD404Drum] ❌ setAudible called but speakerGain is null!');
            return;
        }

        this.isAudible = enabled;
        console.log(`[RD404Drum] setAudible(${enabled})`);

        // Resume AudioContext if needed
        if (enabled && this.audioContext.state === 'suspended') {
            try {
                await this.audioContext.resume();
                console.log('[RD404Drum] AudioContext resumed');
            } catch (error) {
                console.error('[RD404Drum] Failed to resume AudioContext:', error);
                return;
            }
        }
        ...
        console.log(`[RD404Drum] ✅ ${enabled ? 'AUDIBLE' : 'MUTED'}`);
    }


    connect(destination) {
        if (!this.masterGain) {
            console.warn('[RD404Drum] Cannot connect - not initialized');
            return destination;
        }
    ...
    destroy() {
        console.log('[RD404Drum] Destroying...');
        ...
        console.log('[RD404Drum] Destroyed');
    }
    ...
    emit(event, data) {
        ...
        for (const callback of callbacks) {
            try {
                ...
                console.error(`[RD404Drum] Error in ${event} listener:`, error);
            }
        }
    }
}
...
console.log('[RD404Drum] Script executing, window.SynthRegistry:', typeof window.SynthRegistry);
if (typeof window !== "undefined" && window.SynthRegistry) {
    console.log('[RD404Drum] Registering with SynthRegistry');
    window.SynthRegistry.register({
        id: 'rd404',
        name: 'RD404',
        displayName: 'RD404 - Simple Drum Machine',
        description: 'Simple 4-voice drum machine (BD, SD, CH, OH)',
        engineId: 11,
        class: RD404Drum,
        wasmFiles: {
            js: 'synths/rd404-drum.js',
            wasm: 'synths/rd404-drum.wasm'
        },
        category: 'drum',
        getParameterInfo: () => []
    });
    console.log('[RD404Drum] Registration complete');
} else {
    console.error('[RD404Drum] Cannot register - window.SynthRegistry not available');
}
