...
function findWasmBinary() {
  return locateFile("schwungspiel.wasm");
}
...
var ASM_CONSTS = {
  ...
  136077: () => (navigator.mediaDevices === undefined) ? 0 : 1,
  136136: ($0, $1, $2, $3, $4, $5, $6) => {
    const device = $0;
    const w = $1;
    const h = $2;
  ...
  140656: () => Module["SDL3"].audioContext.sampleRate,
  140707: ($0, $1, $2, $3) => {
    var SDL3 = Module["SDL3"];
  ...
  154673: () => window.innerWidth,
  154703: () => window.innerHeight,
  154734: $0 => {
...
function js_midi_init() {
  ...
  navigator.requestMIDIAccess({
    ...
    if (outputs.length > 0) {
      window.midiOutput = outputs[0];
      console.log("MIDI output:", outputs[0].name);
    }
    access.inputs.forEach(function(input) {
      console.log("MIDI input:", input.name);
      input.onmidimessage = function(msg) {
        var data = msg.data;
        if (Module._wasm_midi_in_callback) {
          var ptr = Module._malloc(data.length);
          Module.HEAPU8.set(data, ptr);
          Module._wasm_midi_in_callback(ptr, data.length);
          Module._free(ptr);
        }
      };
    });
  }).catch(function(err) {
    console.error("Web MIDI access denied:", err);
  });
}

function js_midi_send(data, len) {
  if (window.midiOutput) {
    var msg = new Uint8Array(len);
    for (var i = 0; i < len; i++) {
      msg[i] = Module.HEAPU8[data + i];
    }
...
var _main, _free, _malloc, _wasm_midi_in_callback, _wasm_restore_gl_context, _wasm_load_module_callback, _wasm_switch_to_session, _wasm_switch_to_arrange, _SDL_free, _SDL_malloc, _SDL_calloc, _SDL_realloc, _Emscripten_HandlePointerEnter, _Emscripten_HandlePointerLeave, _Emscripten_HandlePointerGeneric, _Emscripten_SendDragEvent, _Emscripten_SendDragCompleteEvent, _Emscripten_SendDragTextEvent, _Emscripten_SendDragFileEvent, _Emscripten_SendSystemThemeChangedEvent, _requestFullscreenThroughSDL, _setThrew, __emscripten_tempret_set, __emscripten_stack_restore, __emscripten_stack_alloc, _emscripten_stack_get_current, ___cxa_free_exception, ___cxa_decrement_exception_refcount, ___cxa_increment_exception_refcount, ___cxa_can_catch, ___cxa_get_exception_ptr, memory, __indirect_function_table, wasmMemory, wasmTable;

function assignWasmExports(wasmExports) {
  _main = Module["_main"] = wasmExports["__main_argc_argv"];
  _free = Module["_free"] = wasmExports["free"];
  _malloc = Module["_malloc"] = wasmExports["malloc"];
  _wasm_midi_in_callback = Module["_wasm_midi_in_callback"] = wasmExports["wasm_midi_in_callback"];
  _wasm_restore_gl_context = Module["_wasm_restore_gl_context"] = wasmExports["wasm_restore_gl_context"];
  _wasm_load_module_callback = Module["_wasm_load_module_callback"] = wasmExports["wasm_load_module_callback"];
  _wasm_switch_to_session = Module["_wasm_switch_to_session"] = wasmExports["wasm_switch_to_session"];
  _wasm_switch_to_arrange = Module["_wasm_switch_to_arrange"] = wasmExports["wasm_switch_to_arrange"];
